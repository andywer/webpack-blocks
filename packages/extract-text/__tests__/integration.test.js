...
test('works with sass()', t => {
  const config = createConfig({}, [
    match('*.scss', [sass(), extractText('css/styles.css')])
  ])
...
test('works with html()', t => {
  const config = createConfig({}, [
    match('*.html', [html(), extractText('build/layout.html')])
  ])

  const id = config.plugins[0].id
  const plugin = Object.assign(new ExtractTextPlugin('build/layout.html'), {
    id
  })
